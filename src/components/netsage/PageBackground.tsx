...
export function PageBackground({ variant }: PageBackgroundProps) {
  ...
  const baseGrid = (
    <div className="absolute inset-0 z-0 bg-[linear-gradient(rgba(56,189,248,0.05)_1px,transparent_1px),linear-gradient(90deg,rgba(56,189,248,0.05)_1px,transparent_1px)] bg-[size:3rem_3rem] [mask-image:radial-gradient(ellipse_100%_100%_at_50%_50%,#000_60%,transparent_100%)]"></div>
  );

  const sharedBlobs = (
    <>
      <div
        className={cn("absolute -top-[15%] -left-[10%] h-[60%] w-[60%] rounded-full blur-[140px]", !reduceMotion && "animate-float-slow")}
        style={{ background: "radial-gradient(circle, rgba(56,189,248,0.30) 0%, rgba(56,189,248,0.12) 45%, transparent 75%)" }}
      />
      <div
        className={cn("absolute -bottom-[12%] -right-[8%] h-[58%] w-[58%] rounded-full blur-[140px]", !reduceMotion && "animate-float-slower")}
        style={{ background: "radial-gradient(circle, rgba(251,146,60,0.28) 0%, rgba(251,146,60,0.10) 45%, transparent 75%)" }}
      />
    </>
  );
    ...
    >
        ...
        <>
          <div className="absolute inset-0 z-0 opacity-[0.04]">
            <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
              <defs>
                <pattern id="dash-net" x="0" y="0" width="80" height="80" patternUnits="userSpaceOnUse">
                  <circle cx="40" cy="40" r="1.5" fill="#38BDF8" />
                  <path d="M40 0 L40 80 M0 40 L80 40" stroke="#38BDF8" strokeWidth="0.4" strokeDasharray="3 5" />
                </pattern>
              </defs>
          ...
          <div
            className={cn("absolute top-[30%] right-[25%] h-[35%] w-[35%] rounded-full blur-[120px]", !reduceMotion && "animate-pulse-slow")}
      ...
      {variant === "audit" && (
        <>
            ...
            <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
              <defs>
                <pattern id="audit-lines" x="0" y="0" width="120" height="40" patternUnits="userSpaceOnUse">
                  <path d="M10 20 h30 M50 20 h60" stroke="#38BDF8" strokeWidth="0.8" strokeDasharray="4 2" />
          ...
          <div className={cn("absolute inset-y-0 left-0 w-full bg-gradient-to-b from-transparent via-sky-400/5 to-transparent", !reduceMotion && "animate-scan-slow")} />
          <div
      ...
      {variant === "analysis" && (
        <>
          ...
          <div
            className={cn("absolute top-[15%] right-[15%] h-[45%] w-[45%] rounded-full blur-[110px]", !reduceMotion && "animate-pulse-slow")}
            style={{ background: "radial-gradient(circle, rgba(56,189,248,0.18) 0%, transparent 70%)" }}
          />
          <div
            className={cn("absolute bottom-[5%] left-[10%] h-[35%] w-[40%] rounded-full blur-[120px]", !reduceMotion && "animate-float-slow")}
            style={{ background: "radial-gradient(circle, rgba(251,146,60,0.14) 0%, transparent 70%)" }}
          />
        </>
      )}
      ...
      {variant === "learning" && (
        <>
          <div className="absolute inset-0 z-0 flex items-center justify-center opacity-[0.06]">
            <div className={cn("h-[600px] w-[600px] rounded-full border border-sky-400 border-dashed", !reduceMotion && "animate-[spin_40s_linear_infinite]")}></div>
            <div className={cn("absolute h-[400px] w-[400px] rounded-full border border-orange-400", !reduceMotion && "animate-[spin_30s_linear_infinite_reverse]")}></div>
          </div>
          <div
      ...
      {variant === "reports" && (
        <>
          <div className="absolute inset-0 z-0 opacity-[0.05]">
            <div className="absolute left-12 top-0 h-full w-px" style={{ background: "linear-gradient(to bottom, transparent, #38BDF8, transparent)" }}></div>
            <div className="absolute left-[calc(100%-3rem)] top-0 h-full w-px" style={{ background: "linear-gradient(to bottom, transparent, #FB923C, transparent)" }}></div>
          </div>
          <div
